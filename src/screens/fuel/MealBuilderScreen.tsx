...
export default function MealBuilderScreen({ navigation, route }: any) {
  ...
  return <ScreenShell title="Build your meal" eyebrow={`${target.carbTarget} G TARGET`} back onBack={() => navigation.goBack()}>
    ...
    {!ingredients.length ? <View style={styles.emptyCard}><Text style={styles.emptyIcon}>＋</Text><Text style={styles.emptyTitle}>Start with one food</Text><Text style={styles.emptyText}>Search above or scan your plate instead.</Text></View> : <View style={styles.items}>{ingredients.map((item) => <View key={item.id} style={styles.item}>
      <Text style={styles.foodEmoji}>{item.food.emoji}</Text><View style={styles.foodCopy}><Text style={styles.foodName}>{item.food.name}</Text><Text style={styles.foodMeta}>{Math.round(item.food.per100g.carbs * item.grams / 100)}g carbs · {item.food.carbSpeed === "unknown" ? "speed unclassified" : item.food.carbSpeed}</Text></View>
    ...
    <View style={styles.compare}>
      ...
      {macros.unclassifiedCarbs > 0 ? <View style={styles.compareRow}><Text style={styles.compareName}>Unclassified</Text><Text style={styles.compareNumber}>—</Text><Text style={styles.compareNumber}>{Math.round(macros.unclassifiedCarbs)}g</Text></View> : null}
      <CarbSpeedBar fast={macros.fastCarbs} medium={macros.mediumCarbs} slow={macros.slowCarbs} unknown={macros.unclassifiedCarbs} />
    </View>
...
const styles = StyleSheet.create({
  ...
  targetMini: { flexDirection: "row", alignItems: "center", gap: 12, backgroundColor: strictlyColors.ink, borderWidth: 1, borderColor: strictlyColors.border, borderRadius: strictlyRadius.large, padding: 15 },
  targetLabel: { fontFamily: strictlyType.semibold, color: strictlyColors.sage, fontSize: 11, letterSpacing: 1.1 },
  targetValue: { fontFamily: strictlyType.bold,  color: strictlyColors.white, fontSize: 20, marginTop: 2 },
  ...
  actual: { minWidth: 42, textAlign: "right", fontFamily: strictlyType.bold,  color: strictlyColors.white, fontSize: 16 },
  nameInput: { height: 50, backgroundColor: strictlyColors.surface, borderWidth: 1, borderColor: strictlyColors.border, borderRadius: strictlyRadius.medium, paddingHorizontal: 14, color: strictlyColors.text, fontFamily: strictlyType.medium, marginTop: 12 },
  ...
  searchInput: { flex: 1, color: strictlyColors.text, fontFamily: strictlyType.regular },
  ...
  noResultsTitle: { fontFamily: strictlyType.bold,  color: strictlyColors.text, fontSize: 15 },
  noResultsText: { fontFamily: strictlyType.regular, color: strictlyColors.textSoft, fontSize: 11, lineHeight: 17, marginTop: 5 },
  ...
  captureText: { fontFamily: strictlyType.bold,  color: strictlyColors.onLime, fontSize: 12 },
  ...
  captureQuickText: { fontFamily: strictlyType.bold,  color: strictlyColors.text, fontSize: 11 },
  foodEmoji: { fontSize: 20, width: 28, textAlign: "center" },
  foodCopy: { flex: 1 },
  foodName: { fontFamily: strictlyType.bold,  color: strictlyColors.text, fontSize: 13 },
  foodMeta: { fontFamily: strictlyType.regular, color: strictlyColors.textSoft, fontSize: 11, marginTop: 3, textTransform: "capitalize" },
  ...
  sectionTitle: { fontFamily: strictlyType.bold,  color: strictlyColors.text, fontSize: 18, marginTop: 24, marginBottom: 9 },
  count: { fontFamily: strictlyType.semibold, color: strictlyColors.textSoft, fontSize: 11 },
  ...
  grams: { color: strictlyColors.text, fontFamily: strictlyType.bold,  fontSize: 12 },
  g: { fontFamily: strictlyType.regular, color: strictlyColors.textSoft, fontSize: 11 },
  ...
  emptyTitle: { fontFamily: strictlyType.bold,  color: strictlyColors.text, marginTop: 8 },
  emptyText: { fontFamily: strictlyType.regular, color: strictlyColors.textSoft, fontSize: 11, marginTop: 4 },
  ...
  compareLabel: { flex: 1, textAlign: "right", fontFamily: strictlyType.semibold, color: strictlyColors.textSoft, fontSize: 11, letterSpacing: 1.1 },
  compareRow: { flexDirection: "row", paddingVertical: 8, borderTopWidth: 1, borderTopColor: strictlyColors.border },
  compareName: { flex: 1, fontFamily: strictlyType.medium, color: strictlyColors.text, fontSize: 12 },
  compareNumber: { flex: 1, textAlign: "right", fontFamily: strictlyType.medium, color: strictlyColors.textSoft, fontSize: 12 },
  onTarget: { color: strictlyColors.good, fontFamily: strictlyType.bold },
  ...
  macroValue: { fontFamily: strictlyType.bold,  color: strictlyColors.text, fontSize: 15 },
  macroLabel: { fontFamily: strictlyType.semibold, color: strictlyColors.textSoft, fontSize: 11, textTransform: "uppercase", marginTop: 3 },
  ...
  source: { fontFamily: strictlyType.regular, color: strictlyColors.textSoft, fontSize: 11, lineHeight: 14, marginTop: 11 },
});
