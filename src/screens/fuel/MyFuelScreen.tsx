...
export default function MyFuelScreen({ navigation }: any) {
  ...
  return <ScreenShell>
    ...
    {meals.slice(0, 8).map((meal) => <TouchableOpacity key={meal.id} onPress={() => navigation.getParent()?.navigate("MealAnalysis", { mealId: meal.id })} style={styles.meal}>
      ...
      <TouchableOpacity style={styles.repeat} onPress={() => { setIngredients(meal.ingredients.map((item) => ({ ...item, id: `${item.id}-repeat-${Date.now()}` }))); navigation.getParent()?.navigate("BuildMeal"); }}><Ionicons name="refresh" size={17} color={strictlyColors.ink} /></TouchableOpacity>
    </TouchableOpacity>)}
    ...
    {!saved.length ? <TouchableOpacity style={styles.savedEmpty} onPress={() => navigation.navigate("Discover")}><Ionicons name="bookmark-outline" size={22} color={strictlyColors.ink} /><Text style={styles.savedEmptyText}>Save useful community meals to find them here.</Text></TouchableOpacity> : null}
  </ScreenShell>;
}

const styles = StyleSheet.create({
  eyebrow: { fontFamily: strictlyType.mono, color: strictlyColors.textSoft, fontSize: 9, letterSpacing: 1.5, marginTop: 9 },
  title: { fontFamily: strictlyType.sansMedium, fontWeight: "900", color: strictlyColors.ink, fontSize: 32, letterSpacing: -1, marginTop: 7 },
  ...
  statValue: { fontFamily: strictlyType.sansMedium, fontWeight: "900", color: strictlyColors.ink, fontSize: 24 },
  ...
  sectionTitle: { fontFamily: strictlyType.sansMedium, fontWeight: "800", color: strictlyColors.ink, fontSize: 18 },
  sectionMeta: { fontFamily: strictlyType.mono, color: strictlyColors.textSoft, fontSize: 8 },
  link: { fontFamily: strictlyType.sansMedium, fontWeight: "700", color: strictlyColors.ink, fontSize: 10 },
  ...
  mealCopy: { flex: 1 },
  mealName: { fontFamily: strictlyType.sansMedium, fontWeight: "800", color: strictlyColors.ink, fontSize: 12 },
  ...
  emptyTitle: { fontFamily: strictlyType.sansMedium, fontWeight: "800", color: strictlyColors.ink, fontSize: 15 },
  ...
  savedEmptyText: { flex: 1, fontFamily: strictlyType.sans, color: strictlyColors.ink, fontSize: 11, lineHeight: 16 },
});
