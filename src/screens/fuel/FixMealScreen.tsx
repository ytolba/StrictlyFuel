...
export default function FixMealScreen({ navigation, route }: any) {
  ...
  fixes.forEach((fix) => {
    if (fix.action === "increase") {
      const index = projected.findIndex((item) => item.id === fix.ingredientId || item.food.id === fix.foodId);
      if (index >= 0) projected[index] = { ...projected[index], grams: projected[index].grams + fix.grams };
    } else if (fix.action === "add" || fix.action === "top_up") {
      const food = FUEL_FOODS.find((item) => item.id === fix.foodId || item.name === fix.ingredientName);
  ...
  return <ScreenShell title="Improve my fuel" eyebrow="ONE NATURAL CHANGE" back onBack={() => navigation.goBack()}>
    ...
    <Text style={styles.intro}>Strictly uses more of what you already have first. A new food is only suggested when it genuinely belongs with the meal.</Text>
    {fixes.length ? fixes.map((fix) => <View key={fix.id} style={styles.fix}><View style={styles.sign}><Text style={styles.signText}>+</Text></View><View style={styles.fixCopy}><Text style={styles.fixTitle}>{fix.action === "increase" ? `Add more ${fix.ingredientName}` : fix.action === "top_up" ? `Top up with ${fix.ingredientName}` : `Add ${fix.ingredientName}`}</Text><Text style={styles.fixAmount}>About +{fix.carbImpact || 0} g carbs</Text><Text style={styles.fixDetail}>{fix.detail}</Text></View></View>) : <View style={styles.already}><Ionicons name="checkmark-circle" size={24} color={strictlyColors.good} /><Text style={styles.alreadyText}>This meal is already close enough. No extra changes needed.</Text></View>}
...
const styles = StyleSheet.create({
  ...
  scoreLabel: { fontFamily: strictlyType.semibold, color: strictlyColors.textSoft, fontSize: 11, letterSpacing: 1.1, textAlign: "center" },
  oldScore: { fontFamily: strictlyType.bold,  color: strictlyColors.textSoft, fontSize: 37, textAlign: "center", marginTop: 4 },
  newScore: { fontFamily: strictlyType.bold,  color: strictlyColors.text, fontSize: 37, textAlign: "center", marginTop: 4 },
  intro: { fontFamily: strictlyType.regular, color: strictlyColors.textSoft, fontSize: 13, lineHeight: 20, marginVertical: 18 },
  ...
  minus: { backgroundColor: strictlyColors.dangerSurface },
  signText: { fontFamily: strictlyType.bold,  color: strictlyColors.onLime, fontSize: 20 },
  fixCopy: { flex: 1 },
  fixTitle: { fontFamily: strictlyType.bold,  color: strictlyColors.text, fontSize: 14 },
  fixAmount: { fontFamily: strictlyType.semibold, color: strictlyColors.text, fontSize: 11, marginTop: 4 },
  fixDetail: { fontFamily: strictlyType.regular, color: strictlyColors.textSoft, fontSize: 11, lineHeight: 16, marginTop: 6 },
  already: { flexDirection: "row", gap: 10, padding: 16, backgroundColor: strictlyColors.surface, borderRadius: strictlyRadius.large },
  alreadyText: { flex: 1, fontFamily: strictlyType.regular, color: strictlyColors.textSoft, fontSize: 12, lineHeight: 18 },
  nextTime: { padding: 16, marginTop: 10, borderRadius: strictlyRadius.large, backgroundColor: strictlyColors.cream }, nextTimeLabel: { fontFamily: strictlyType.semibold, color: strictlyColors.textSoft, fontSize: 11, letterSpacing: 1.1, marginBottom: 8 }, nextTimeRow: { flexDirection: "row", gap: 8, marginTop: 6 }, nextTimeText: { flex: 1, fontFamily: strictlyType.regular, color: strictlyColors.text, fontSize: 11, lineHeight: 16 },
  preview: { padding: 16, backgroundColor: strictlyColors.ink, borderWidth: 1, borderColor: strictlyColors.border, borderRadius: strictlyRadius.large, marginTop: 13 },
  previewLabel: { fontFamily: strictlyType.semibold, color: strictlyColors.sage, fontSize: 11, letterSpacing: 1.1, marginBottom: 7 },
  previewRow: { flexDirection: "row", justifyContent: "space-between", paddingVertical: 7, borderTopWidth: 1, borderTopColor: strictlyColors.overlayLine },
  previewName: { fontFamily: strictlyType.regular, color: strictlyColors.inverseTextSoft, fontSize: 11 },
  previewValue: { fontFamily: strictlyType.bold,  color: strictlyColors.inverseText, fontSize: 12 },
  ...
  keepText: { fontFamily: strictlyType.medium, color: strictlyColors.textSoft, fontSize: 12 },
});
